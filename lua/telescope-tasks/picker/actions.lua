...
function actions.select_task(prompt_bufnr)
  ...
  executor.run(
    task,
    function() refresh_picker() end,
    function() refresh_picker(prompt_bufnr) end,
    true,
    false
  )
end
...
function actions.selected_task_output(prompt_bufnr)
  ...
  output.open(
    selection.value,
    function() telescope_actions.close(prompt_bufnr) end
  )
end
...
function actions.delete_selected_task_output(picker_buf)
  ...
  if not running then refresh_picker(picker_buf, true, false) end
end
...
function actions.toggle_last_output() output.toggle_last() end
...
refresh_picker = function(picker_buf, close_on_no_results)
  ...
  if not ok and type(e) == "string" then util.error(e) end
end

refresh_previewer = function(picker_buf)
  local buf = vim.api.nvim_get_current_buf()
  local p = action_state.get_current_picker(picker_buf or buf)
  if p ~= nil then p:refresh_previewer() end
end
